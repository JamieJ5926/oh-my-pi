...
export const SETTINGS_SCHEMA = {
	...
	"task.softRequestBudget": {
		...
		ui: {
			...
			description:
				"Soft per-subagent request budget (assistant requests per run). Crossing it injects a wrap-up steering notice (see task.softRequestBudgetNotice); at 1.5x the budget the run is force-stopped and the agent must yield its partial findings. 0 disables the guard. Bundled scout/sonic agents cap out at a lower built-in budget while poteto-agent-deep caps higher at a built-in budget of 400, so the setting can only lower an agent's budget, never raise it above its bundled entry.",
			options: [
				{ value: "0", label: "Disabled" },
