...
export type TransportDeliveryState =
  ...
  | { readonly kind: "stale-generation"; readonly messageId: string; readonly expected: number; readonly actual?: number }
...
async function readJournal(file: string): Promise<JournalEntry[]> {
  ...
  for (const line of text.split("\n")) {
    ...
    try {
      ...
      entries.push({ envelope: parseMessageEnvelope(row.envelope), attempts: row.attempts, nextAttemptAt: row.nextAttemptAt });
    } catch {
...
async function compactJournal(file: string, entries: readonly JournalEntry[], removedIds: readonly string[] = []): Promise<void> {
...
function enqueueJournalCompaction(file: string, entries: readonly JournalEntry[], removedIds: Set<string>): Promise<void> {
...
interface WireRegister { readonly type: "register"; readonly address: string; }
interface WireUnregister { readonly type: "unregister"; readonly address: string; }
interface WireSend { readonly type: "send"; readonly envelope: unknown; }
interface WireAck { readonly type: "ack"; readonly messageId: string; }
interface WireDelivery { readonly type: "delivery"; readonly envelope: unknown; }
interface WireAccepted { readonly type: "accepted"; readonly messageId: string; }
interface WireReject { readonly type: "reject"; readonly messageId?: string; readonly reason: string; readonly expected?: number; readonly actual?: number; }
...
export class LocalTransportAdapter implements TransportAdapter {
  static readonly #handlers = new Map<string, { readonly address: SessionAddress; readonly handler: TransportHandler; readonly owner: LocalTransportAdapter }>();
  ...
  async send(envelope: MessageEnvelope): Promise<TransportDeliveryState> {
    ...
    if (size > this.#limits.maxEnvelopeBytes) return { kind: "backpressure", messageId: id, bytes: size, limit: this.#limits.maxEnvelopeBytes };
    ...
    if (target && generationOf(target.address) !== generationOf(envelope.destination)) {
      return { kind: "stale-generation", messageId: id, expected: generationOf(envelope.destination), actual: generationOf(target.address) };
    }
    if (target) {
  ...
  async #queue(envelope: MessageEnvelope, attempts: number): Promise<TransportDeliveryState> {
    ...
    if (currentBytes + size > limit) return { kind: "backpressure", messageId: id, bytes: currentBytes + size, limit };
    const entry: JournalEntry = { envelope, attempts, nextAttemptAt: Date.now() };
    this.#pending.set(id, entry);
    this.#journalWrite = this.#journalWrite.then(() => enqueueJournalOperation(this.#journalPath, () => appendJournal(this.#journalPath, entry)));
      ...
      for (const [id, entry] of this.#pending) {
        ...
        try {
          ...
          if (attempts >= this.#limits.maxAttempts) {
            ...
            const updated: JournalEntry = { envelope: entry.envelope, attempts, nextAttemptAt: retryAt(entry, this.#limits) };
            this.#pending.set(id, updated);
            nextAt = Math.min(nextAt, updated.nextAttemptAt);
          }
        }
      }
      this.#journalWrite = this.#journalWrite.then(() => enqueueJournalCompaction(this.#journalPath, [...this.#pending.values()], this.#removed));
      await this.#journalWrite;
      if (nextAt < Number.POSITIVE_INFINITY && !this.#closed) {
        if (this.#drainTimer) clearTimeout(this.#drainTimer);
        this.#drainTimer = setTimeout(() => { this.#drainTimer = undefined; void this.#drain(); }, Math.max(1, nextAt - Date.now()));
      }
    } finally {
      this.#draining = false;
    }
...
export class BrokerBackedTransportAdapter implements TransportAdapter {
  ...
  constructor(options: { readonly socketPath: string; readonly journalPath: string; readonly limits?: TransportLimits }) {
  ...
  #onWire(message: WireMessage): void {
    if (message.type === "delivery") {
      ...
      try { envelope = parseMessageEnvelope(message.envelope); } catch { return; }
      ...
      void Promise.resolve(handler.handler(envelope)).then(() => {
    ...
    if (message.type === "accepted") {
      ...
      this.#journalWrite = this.#journalWrite.then(() => enqueueJournalOperation(this.#journalPath, () => compactJournal(this.#journalPath, [...this.#pending.values()])));
  ...
  async #flush(): Promise<void> {
    ...
    const now = Date.now();
    for (const handler of this.#handlers.values()) this.#write({ type: "register", address: formatSessionAddress(handler.address) });
    let nextAt = Number.POSITIVE_INFINITY;
    for (const [id, entry] of this.#pending) {
      ...
      const updated: JournalEntry = { envelope: entry.envelope, attempts: entry.attempts + 1, nextAttemptAt: retryAt(entry, this.#limits) };
      this.#pending.set(id, updated);
      nextAt = Math.min(nextAt, updated.nextAttemptAt);
    }
    this.#journalWrite = this.#journalWrite.then(() => enqueueJournalOperation(this.#journalPath, () => compactJournal(this.#journalPath, [...this.#pending.values()])));
    ...
    if (nextAt < Number.POSITIVE_INFINITY && !this.#closed) {
      if (this.#drainTimer) clearTimeout(this.#drainTimer);
      this.#drainTimer = setTimeout(() => { this.#drainTimer = undefined; void this.#flush(); }, Math.max(1, nextAt - Date.now()));
    }
  }
...
export class BrokerTransportServer {
    ...
    this.#redelivery = this.#redelivery.then(async () => {
      ...
      for (const [id, entry] of this.#pending) {
        ...
        if (entry.attempts >= this.#limits.maxAttempts) {
          this.#pending.delete(id);
          await appendDeadLetter(this.#deadLetterPath, { envelope: entry.envelope, attempts: entry.attempts, reason: "maximum delivery attempts exceeded", deadLetteredAt: Date.now() });
          changed = true;
          continue;
        }
        const target = this.#registrations.get(addressKey(entry.envelope.destination));
        if (!target || generationOf(target.address) !== generationOf(entry.envelope.destination) || !target.socket.writable) continue;
  ...
  async #deliverPendingFor(key: string): Promise<void> {
    ...
    let changed = false;
    for (const [id, entry] of this.#pending) {
      if (addressKey(entry.envelope.destination) !== key || generationOf(entry.envelope.destination) !== generationOf(target.address) || entry.attempts >= this.#limits.maxAttempts || entry.nextAttemptAt > now) continue;
      target.socket.write(encodeWire({ type: "delivery", envelope: entry.envelope }));
      this.#pending.set(id, { envelope: entry.envelope, attempts: entry.attempts + 1, nextAttemptAt: retryAt(entry, this.#limits) });
      changed = true;
    }
    if (!changed) {
      this.#scheduleRetry();
  ...
  async listen(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onListening = (): void => { this.#server.off("error", onError); this.#listening = true; resolve(); };
  ...
  #accept(socket: net.Socket): void {
    ...
    const onClose = (): void => {
      ...
      for (const [key, registration] of this.#registrations) if (registration.socket === socket) this.#registrations.delete(key);
    ...
    socket.on("data", chunk => {
      ...
      for (;;) {
        ...
        try { void this.#handle(socket, parseWire(JSON.parse(line))); }
        catch { socket.write(encodeWire({ type: "reject", reason: "invalid frame" })); }
      }
    });
  ...
  async #handle(socket: net.Socket, message: WireMessage): Promise<void> {
    if (message.type === "register") {
      ...
      try { address = parseSessionAddress(message.address); } catch { socket.write(encodeWire({ type: "reject", reason: "invalid address" })); return; }
      const key = addressKey(address);
      const prior = this.#registrations.get(key);
      if (prior && prior.socket !== socket && generationOf(prior.address) >= generationOf(address)) {
        socket.write(encodeWire({ type: "reject", reason: "recipient already registered", expected: generationOf(address), actual: generationOf(prior.address) }));
        return;
      }
    ...
    if (message.type === "unregister") {
      try {
        ...
        if (current?.socket === socket && generationOf(current.address) === generationOf(address)) this.#registrations.delete(key);
      } catch { /* Ignore an unregister for an invalid/old address. */ }
      return;
    }
    if (message.type === "ack") {
      if (!this.#pending.delete(message.messageId)) return;
    ...
    let envelope: MessageEnvelope;
    try { envelope = parseMessageEnvelope(message.envelope); } catch { socket.write(encodeWire({ type: "reject", reason: "invalid envelope" })); return; }
    const id = envelope.id;
    const size = serializedBytes(envelope);
    ...
    const target = this.#registrations.get(key);
    if (target && generationOf(target.address) !== generationOf(envelope.destination)) {
      socket.write(encodeWire({ type: "reject", messageId: id, reason: "stale generation", expected: generationOf(envelope.destination), actual: generationOf(target.address) }));
      return;
    }
    const seen = this.#seen.get(key) ?? new Set<string>();
    ...
    if (this.#journalBytes + Buffer.byteLength(journalLine({ envelope, attempts: 0, nextAttemptAt: Date.now() }), "utf8") > this.#limits.maxJournalBytes) {
    ...
    this.#journalWrite = this.#journalWrite.then(() => enqueueJournalOperation(this.#journalPath, () => appendJournal(this.#journalPath, entry)));
    await this.#journalWrite;
    if (target?.socket.writable && generationOf(target.address) === generationOf(envelope.destination)) await this.#deliverPendingFor(key);
    socket.write(encodeWire({ type: "accepted", messageId: id }));
  }
