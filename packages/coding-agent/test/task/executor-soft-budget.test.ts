...
import {
	resolveConfiguredDefaultBudget,
...
describe("resolveConfiguredDefaultBudget", () => {
	it("falls back to the agent entry when task.softRequestBudget is unset", () => {
		...
		expect(
			resolveSoftRequestBudget("poteto-agent-deep", resolveConfiguredDefaultBudget("poteto-agent-deep", settings)),
		).toBe(400);
	...
	it("passes an explicitly configured value through, including 0 to disable", () => {
		expect(
			resolveConfiguredDefaultBudget("poteto-agent-deep", Settings.isolated({ "task.softRequestBudget": 50 })),
		).toBe(50);
		expect(
			resolveSoftRequestBudget(
				"poteto-agent-deep",
		...
		expect(
			resolveSoftRequestBudget(
				"poteto-agent-deep",
				resolveConfiguredDefaultBudget("poteto-agent-deep", Settings.isolated({ "task.softRequestBudget": 0 })),
			),
		).toBe(0);
	});
